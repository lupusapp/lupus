...
struct _LupusMainHeaderBar {
    ...
    GtkMenuItem *profile_popover_none, *profile_popover_away, *profile_popover_busy, *profile_popover_toxid;
    GtkPopover *popover;
...
static gboolean profile_button_press_event(LupusMainHeaderBar *instance, GdkEvent *event)
{
...
{
    lupus_editablelabel_set_value(instance->name, lupus_wrapper_get_name(lupus_wrapper));
}

static void wrapper_notify_status_message_cb(LupusMainHeaderBar *instance)
{
    lupus_editablelabel_set_value(instance->status_message, lupus_wrapper_get_status_message(lupus_wrapper));
}
...
static void wrapper_notify_status_cb(LupusMainHeaderBar *instance)
{
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(GTK_WIDGET(instance->profile)), class_name[status]);
    gtk_style_context_add_class(gtk_widget_get_style_context(GTK_WIDGET(instance->profile_bigger)), class_name[status]);
}
...
static void wrapper_notify_connection_cb(LupusMainHeaderBar *instance)
{
    ...
    for (gsize i = 0, j = G_N_ELEMENTS(offset); i < j; ++i) {
        gtk_widget_set_sensitive(GTK_WIDGET(*(gpointer *)((gchar *)instance + offset[i])), TRUE);
    }
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(GTK_WIDGET(instance->profile_bigger)), "profile--offline");
}
...
static void toxid_cb()
{
    ...
    if (!label) {
        ...
        label = g_object_new(GTK_TYPE_LABEL, "label", address, "selectable", TRUE, NULL);
    ...
    if (!dialog) {
        dialog = g_object_new(GTK_TYPE_DIALOG, "use-header-bar", TRUE, "title", "ToxID", "resizable", FALSE, NULL);
...
{
    GtkMenuItem **item[] = {&instance->profile_popover_none, &instance->profile_popover_away,
                            &instance->profile_popover_busy, &instance->profile_popover_toxid};
    gchar *svg[] = {LUPUS_RESOURCES "/status_none.svg", LUPUS_RESOURCES "/status_away.svg",
    ...
    for (gsize i = 0, j = G_N_ELEMENTS(label); i < j; ++i) {
        ...
        gtk_box_pack_start(box, gtk_image_new_from_resource(svg[i]), FALSE, TRUE, 0);
        ...
        *item[i] = GTK_MENU_ITEM(gtk_menu_item_new());
        ...
        gtk_menu_shell_append(GTK_MENU_SHELL(instance->profile_popover), GTK_WIDGET(*item[i]));

        if (i == (j - 2)) {
            gtk_menu_shell_append(GTK_MENU_SHELL(instance->profile_popover), gtk_separator_menu_item_new());
        }
...
static void menu_popover_about_activate_cb()
{
    ...
    gtk_show_about_dialog(NULL, "authors", authors, "license_type", GTK_LICENSE_MIT_X11, "logo",
                          gdk_pixbuf_new_from_resource(LUPUS_RESOURCES "/lupus.svg", NULL), "program-name", "Lupus",
                          "version", LUPUS_VERSION, "website", "https://github.com/LupusApp/Lupus", "website-label",
                          "Github", "wrap-license", TRUE, NULL);
}

static void init_menu_popover(LupusMainHeaderBar *instance)
{
    GtkBox *box = GTK_BOX(gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0));
    gtk_box_pack_start(box, gtk_image_new_from_icon_name("help-about", GTK_ICON_SIZE_MENU), FALSE, TRUE, 0);
    ...
    gtk_menu_shell_append(GTK_MENU_SHELL(instance->menu_popover), GTK_WIDGET(about_menu));

    gtk_widget_show_all(GTK_WIDGET(instance->menu_popover));

    g_signal_connect_swapped(about_menu, "activate", G_CALLBACK(menu_popover_about_activate_cb), instance);
}

static void active_chat_friend_notify_name_cb(LupusMainHeaderBar *instance)
{
    LupusWrapperFriend *friend =
        lupus_wrapper_get_friend(lupus_wrapper, lupus_wrapper_get_active_chat_friend(lupus_wrapper));

    gtk_header_bar_set_title(instance->right_headerbar, lupus_wrapperfriend_get_name(friend));
}

static void active_chat_friend_notify_status_message_cb(LupusMainHeaderBar *instance)
{
    LupusWrapperFriend *friend =
        lupus_wrapper_get_friend(lupus_wrapper, lupus_wrapper_get_active_chat_friend(lupus_wrapper));

    gtk_header_bar_set_subtitle(instance->right_headerbar, lupus_wrapperfriend_get_status_message(friend));
}
...
static void wrapper_notify_active_chat_friend_cb(LupusMainHeaderBar *instance)
{
    ...
    if (instance->active_chat_friend_notify_name_handler_id) {
        g_signal_handler_disconnect(instance, instance->active_chat_friend_notify_name_handler_id);
    }
    g_signal_connect_swapped(friend, "notify::name", G_CALLBACK(active_chat_friend_notify_name_cb), instance);

    if (instance->active_chat_friend_notify_status_message_handler_id) {
        g_signal_handler_disconnect(instance, instance->active_chat_friend_notify_status_message_handler_id);
    }
    g_signal_connect_swapped(friend, "notify::status-message", G_CALLBACK(active_chat_friend_notify_status_message_cb),
                             instance);
}

static void file_chooser_dialog_update_preview_cb(GtkFileChooser *chooser, GtkWidget *preview)
{
    ...
    GdkPixbuf *pixbuf = gdk_pixbuf_new_from_file_at_size(filename, FILE_CHOOSER_DIALOG_PREVIEW_SIZE,
...
static void profile_bigger_clicked_cb(LupusMainHeaderBar *instance)
{
    ...
    if (!chooser) {
        chooser = GTK_FILE_CHOOSER(gtk_file_chooser_dialog_new(
            "Select avatar", GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(instance))), GTK_FILE_CHOOSER_ACTION_OPEN,
            "Select", GTK_RESPONSE_ACCEPT, "Cancel", GTK_RESPONSE_CANCEL, NULL));
        ...
        g_signal_connect(chooser, "update-preview", G_CALLBACK(file_chooser_dialog_update_preview_cb), preview);
    }
    ...
    if (gtk_dialog_run(GTK_DIALOG(chooser)) == GTK_RESPONSE_ACCEPT) {
        ...
        if (file_stat.st_size > AVATAR_MAX_FILE_SIZE) {
            lupus_error("Avatar maximum size is <b>%d</b>.", AVATAR_MAX_FILE_SIZE);
            goto end;
        }
        ...
        gchar *avatar_filename =
            g_strconcat(avatars_directory, lupus_wrapper_get_public_key(lupus_wrapper), ".png", NULL);
        g_free(avatars_directory);
        if (g_file_test(avatar_filename, G_FILE_TEST_EXISTS)) {
            ...
            sodium_bin2hex(new_avatar_hash_hex, sizeof(new_avatar_hash_hex), (guint8 *)new_avatar_hash,
                           sizeof(new_avatar_hash));

            if (g_strcmp0(new_avatar_hash_hex, lupus_wrapper_get_avatar_hash(lupus_wrapper)) == 0) {
         ...
         */
        cairo_surface_t *image = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, AVATAR_SIZE, AVATAR_SIZE);
...
static void wrapper_notify_avatar_hash_cb(LupusMainHeaderBar *instance)
{
    ...
    gtk_image_set_from_pixbuf(instance->profile_image,
                              gdk_pixbuf_new_from_file_at_size(filename, AVATAR_SIZE_MINI, AVATAR_SIZE_MINI, NULL));
...
static void lupus_mainheaderbar_constructed(GObject *object)
{
    ...
    instance->name = lupus_editablelabel_new(lupus_wrapper_get_name(lupus_wrapper), tox_max_name_length());
    instance->status_message =
        lupus_editablelabel_new(lupus_wrapper_get_status_message(lupus_wrapper), tox_max_status_message_length());

    gtk_box_pack_start(instance->vbox, GTK_WIDGET(instance->name), FALSE, FALSE, 0);
    gtk_box_pack_start(instance->vbox, GTK_WIDGET(instance->status_message), FALSE, FALSE, 0);
    gtk_box_reorder_child(instance->vbox, GTK_WIDGET(instance->name), 0);

    g_signal_connect(instance->name, "submit", G_CALLBACK(name_submit_cb), NULL);
    g_signal_connect(instance->status_message, "submit", G_CALLBACK(status_message_submit_cb), NULL);
    g_signal_connect_swapped(instance->profile_bigger, "clicked", G_CALLBACK(profile_bigger_clicked_cb), instance);
    ...
    GObjectClass *object_class = G_OBJECT_CLASS(lupus_mainheaderbar_parent_class); // NOLINT
...
static void lupus_mainheaderbar_class_init(LupusMainHeaderBarClass *class)
{
    ...
    gtk_widget_class_set_template_from_resource(widget_class, LUPUS_RESOURCES "/mainheaderbar.ui");
    gtk_widget_class_bind_template_child(widget_class, LupusMainHeaderBar, left_headerbar);
    gtk_widget_class_bind_template_child(widget_class, LupusMainHeaderBar, right_headerbar);
    gtk_widget_class_bind_template_child(widget_class, LupusMainHeaderBar, profile);
    gtk_widget_class_bind_template_child(widget_class, LupusMainHeaderBar, profile_image);
    gtk_widget_class_bind_template_child(widget_class, LupusMainHeaderBar, profile_popover);
    gtk_widget_class_bind_template_child(widget_class, LupusMainHeaderBar, popover);
    gtk_widget_class_bind_template_child(widget_class, LupusMainHeaderBar, profile_bigger);
    gtk_widget_class_bind_template_child(widget_class, LupusMainHeaderBar, profile_bigger_image);
    gtk_widget_class_bind_template_child(widget_class, LupusMainHeaderBar, vbox);
    gtk_widget_class_bind_template_child(widget_class, LupusMainHeaderBar, menu_popover);

    GObjectClass *object_class = G_OBJECT_CLASS(class); // NOLINT
    object_class->constructed = lupus_mainheaderbar_constructed;
}
...
static void lupus_mainheaderbar_init(LupusMainHeaderBar *instance)
{
    ...
    g_signal_connect_swapped(lupus_wrapper, "notify::name", G_CALLBACK(wrapper_notify_name_cb), instance);
    g_signal_connect_swapped(lupus_wrapper, "notify::status_message", G_CALLBACK(wrapper_notify_status_message_cb),
                             instance);
    g_signal_connect_swapped(lupus_wrapper, "notify::status", G_CALLBACK(wrapper_notify_status_cb), instance);
    g_signal_connect_swapped(lupus_wrapper, "notify::connection", G_CALLBACK(wrapper_notify_connection_cb), instance);
    g_signal_connect_swapped(lupus_wrapper, "notify::avatar-hash", G_CALLBACK(wrapper_notify_avatar_hash_cb), instance);

    g_signal_connect_swapped(instance->profile, "clicked", G_CALLBACK(gtk_popover_popup), instance->popover);
    g_signal_connect_swapped(instance->profile, "button-press-event", G_CALLBACK(profile_button_press_event), instance);
    g_signal_connect_swapped(lupus_wrapper, "notify::active-chat-friend",
                             G_CALLBACK(wrapper_notify_active_chat_friend_cb), instance);
}

LupusMainHeaderBar *lupus_mainheaderbar_new(void) { return g_object_new(LUPUS_TYPE_MAINHEADERBAR, NULL); }