...
static void lupus_editablelabel_set_property(GObject *object, guint property_id,
                                             GValue const *value,
                                             GParamSpec *pspec) {
...
static void lupus_editablelabel_class_init(LupusEditableLabelClass *class) {
    ...
    gtk_widget_class_set_template_from_resource(widget_class, LUPUS_RESOURCES
                                                "/editablelabel.ui");
    gtk_widget_class_bind_template_child(widget_class, LupusEditableLabel,
                                         label);
    gtk_widget_class_bind_template_child(widget_class, LupusEditableLabel,
                                         popover);
    gtk_widget_class_bind_template_child(widget_class, LupusEditableLabel,
    ...
    obj_properties[PROP_VALUE] =
        g_param_spec_string("value", "Value", "Label value", "", param);
    obj_properties[PROP_MAX_LENGTH] =
        g_param_spec_uint("max-length", "Max Length", "Label maximum length.",
                          MIN_LENGTH, MAX_LENGTH, MIN_LENGTH, param);

    g_object_class_install_properties(G_OBJECT_CLASS(class), // NOLINT
                                      N_PROPERTIES, obj_properties);

    signals[SUBMIT] = g_signal_new("submit", LUPUS_TYPE_EDITABLELABEL,
...
static void lupus_editablelabel_init(LupusEditableLabel *instance) {
    ...
    g_signal_connect_swapped(instance->submit, "clicked",
                             G_CALLBACK(clicked_cb), instance);
}
